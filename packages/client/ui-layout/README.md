...
Under an embedded carrier the frame is a two-column shell: when the document was booted with `window.__DSH_TRANSPORT__` installed by a native host (the VSCode panel), `createLayoutStore` freezes `embedded` at creation, AppFrame drops the sidebar column, its handle, and every sidebar-only decision, and session navigation belongs to that host UI. The sidebar slot stays declared — its registrants keep mounting harmlessly — while plain browser boots render the unchanged three-column shape.
