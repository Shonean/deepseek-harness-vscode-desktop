...
内嵌载体下框架是两栏形态：当 document 启动时已由宿主装好 `window.__DSH_TRANSPORT__`（VSCode 面板如此），`createLayoutStore` 会在创建时冻结 `embedded`，AppFrame 去掉侧边栏列、对应拖柄与一切侧栏专属决策，会话导航交给宿主 UI。sidebar slot 保持声明状态——注册方继续无害挂载——而普通浏览器启动仍渲染不变的三栏形态。
